      ...
      if (progress >= 100) {
        ...
        if (dependencyName === 'oss') {
          try {
            ...
            await fs.writeFile(path.join(modelDir, 'model.info'), 
              JSON.stringify({
                name: 'OpenAI OSS 2B',
...
app.post('/api/transcribe', async (req, res) => {
  try {
    ...
    try {
      // First, validate the audio file using FFmpeg
      console.log('Validating audio file...');
      const validateCommand = `ffmpeg -v quiet -i "${audioFilePath}" -f null - 2>&1`;
      
      try {
        await execAsync(validateCommand);
        console.log('Audio file validation passed');
      } catch (validateError) {
        console.error('Audio file validation failed:', validateError.message);
        await fs.unlink(audioFilePath);
        return res.status(400).json({
          success: false,
          error: 'Invalid audio file format or corrupted data'
        });
      }
      
      // Use Python's whisper command line tool
